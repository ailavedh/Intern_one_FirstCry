...
async function initializeSchema() {
  if (dbType === 'postgres') {
    ...
    await query(`CREATE TABLE IF NOT EXISTS activities (id SERIAL PRIMARY KEY, child_id INT NOT NULL REFERENCES children(id) ON DELETE CASCADE, teacher_id INT NOT NULL REFERENCES users(id) ON DELETE CASCADE, type VARCHAR(100) NOT NULL, description TEXT NOT NULL, photo_url TEXT, timestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP);`);
    ...
    await query(`CREATE TABLE IF NOT EXISTS feedbacks (id SERIAL PRIMARY KEY, parent_id INT NOT NULL REFERENCES users(id) ON DELETE CASCADE, recipient_id INT NOT NULL REFERENCES users(id) ON DELETE CASCADE, child_id INT REFERENCES children(id) ON DELETE CASCADE, message TEXT NOT NULL, timestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP);`);
    try { await query(`ALTER TABLE activities ALTER COLUMN photo_url TYPE TEXT;`); } catch(e) {}
  } else {
    ...
    await query(`CREATE TABLE IF NOT EXISTS feedbacks (id INTEGER PRIMARY KEY AUTOINCREMENT, parent_id INTEGER NOT NULL, recipient_id INTEGER NOT NULL, child_id INTEGER, message TEXT NOT NULL, timestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (parent_id) REFERENCES users(id) ON DELETE CASCADE, FOREIGN KEY (recipient_id) REFERENCES users(id) ON DELETE CASCADE, FOREIGN KEY (child_id) REFERENCES children(id) ON DELETE CASCADE);`);
  }
